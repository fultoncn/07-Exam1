    ...
    for k in range(b + 1):
        if k >= a:
            if product_of_digits(k) <= threshold:
                total = total + product_of_digits(k)
    return total
...
def problem1b(r):
    ...
    for k in range(new + 1):
        total = total + product_of_digits(r + (new * k))
    return total
